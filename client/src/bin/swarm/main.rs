...
use vek::*;
use Hobania_client::{addr::ConnectionArgs, Client};
...
fn run_client(
    ...
    let mut tick = |client: &mut Client| -> Result<(), thp_client::Error> {
        clock.tick();
...
fn position(index: u32, opt: Opt) -> Vec3<f32> {
    ...
    let movement_offset: Vec2<_> = if opt.movement {
        // blocks per second
        const SPEED: f32 = 9.0; // typical super fast hobania walking speed
